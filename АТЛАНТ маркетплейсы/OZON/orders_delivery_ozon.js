 ...
 */
function getOzonDeliveryOrders() {
  ...
  try {
    ...
    if (!orders || !orders.result || !orders.result.postings || orders.result.postings.length === 0) {
      Logger.log('Нет заказов в статусе "delivering"');
      SpreadsheetApp.getActive().toast('Нет заказов в статусе "delivering"');
      return;
    }
